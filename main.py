...
# Never let user secrets reach the backup repo. `git add -A` used to push
# every hosted bot's .env — i.e. every user's BOT_TOKEN — plus Telethon/
# Pyrogram .session files, which are full ACCOUNT access, not just a bot.
# One public (or later-made-public) backup repo would expose all of them.
_SECRET_GITIGNORE = """# Auto-generated by MasterBot — do not remove.
# Never back up user secrets.
.env
.env.*
*.session
*.session-journal
...
def _write_backup_gitignore():
    """Keep a hard exclude-list inside the backup repo."""
    try:
        HOSTED_DIR.mkdir(parents=True, exist_ok=True)
        gi = HOSTED_DIR / ".gitignore"
        current = gi.read_text(errors="replace") if gi.exists() else ""
        if current != _SECRET_GITIGNORE:
            gi.write_text(_SECRET_GITIGNORE, encoding="utf-8")
    except OSError as e:
        log.warning("Could not write backup .gitignore: %s", e)


def _push_sync(msg="Update"):
    if not GITHUB_PAT: return
    _write_backup_gitignore()
    # --- untrack anything secret that earlier versions already committed ---
    _git(f'cd "{HOSTED_DIR}" && git rm -r --cached --ignore-unmatch -q '
         f'"*.env" "*.session" "*.session-journal" "*.db" "*.log" 2>/dev/null')
...
async def cmd_audit(update:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(uid):
        await update.message.reply_text("❌ Admin only."); return
    sm=await update.message.reply_text("🔒 Running security audit…")

    def _scan():
        rows=[]
        # 1. Are bots isolated from root?
        rows.append(("Bots run unprivileged",
                     bool(_BOT_IDS),
                     f"as '{BOT_USER}'" if _BOT_IDS else "RUNNING AS ROOT — bots can read every secret"))
        # 2. Master secrets readable by bots?
        envp = Path(__file__).resolve().parent/".env"
        try: m = envp.stat().st_mode & 0o077
        except OSError: m = 0
        rows.append((".env private", m==0,
                     "only owner" if m==0 else "readable by others!"))
        # 3. Backup repo excludes secrets?
        gi = HOSTED_DIR/".gitignore"
        ok_gi = gi.exists() and ".env" in gi.read_text(errors="replace")
        rows.append(("Backup excludes secrets", ok_gi or not GITHUB_PAT,
                     "ok" if ok_gi else ("no backup configured" if not GITHUB_PAT
                                          else "BACKUP MAY CONTAIN USER TOKENS")))
        # 4. Are user tokens still tracked in the backup repo?
        tracked=""
        if (HOSTED_DIR/".git").exists():
            try:
                r=subprocess.run("git ls-files | grep -E '\\.env$|\\.session$' | head -5",
                                 cwd=str(HOSTED_DIR), shell=True,
                                 capture_output=True, text=True, timeout=20)
                tracked=r.stdout.strip()
            except Exception: pass
        rows.append(("No secrets tracked in git", not tracked,
                     "clean" if not tracked else f"{len(tracked.splitlines())}+ secret files tracked!"))
        # 5. How many hosted bots carry a token on disk
        n_tok=0
        if HOSTED_DIR.exists():
            for d in HOSTED_DIR.iterdir():
                if d.is_dir() and not d.name.startswith(("_",".")):
                    if _find_tokens(d): n_tok+=1
        return rows, n_tok, tracked
    rows, n_tok, tracked = await asyncio.to_thread(_scan)

    lines=["🔒 *Security Audit*\n"]
    for name, ok, note in rows:
        lines.append(f"{'✅' if ok else '🔴'} {name}\n   _{mdq(note)}_")
    lines.append(f"\n🔑 Bots with a token stored on disk: `{n_tok}`")
    if tracked:
        lines.append("\n⚠️ *Secret files found in the backup repo:*\n```\n"
                     + tracked[:400] + "\n```\nIf that repo was ever public, "
                     "those tokens must be revoked.")
    await sm.edit_text("\n".join(lines)[:4000],parse_mode="Markdown")
